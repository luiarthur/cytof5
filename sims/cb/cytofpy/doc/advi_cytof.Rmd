...
## Minibatches
Since we need to impute missing values, one approach to handling minibatches
could be to impute values only in the current minibatch. I suspect initial
values will have a strong affect on the optimizer, so they should be chosen
carefully. What we currently do to initialize should be fine. Also, updating
only some of the missing observations at a time may lead to slower convergence.
So we should do a full update of all parameters every so often if necessary. This is
my intuition, I have been looking for supporting papers for imputing missing 
values in minibatches, but haven't yet found anything.

An alternative to imputing missing values could be to treat missing observations
as censored (perhaps at 0). What are your thoughts?
