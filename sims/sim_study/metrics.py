...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        sim_dirs = list(filter(lambda d: 'KMCMC' in d, sim_dirs))
        sim_dirs = list(filter(lambda d: not re.search(excludes, d), sim_dirs))

        Nfac = np.array([int(re.findall(r'(?<=Nfac)\d+', d)[0])
                      for d in sim_dirs])
        Nfac = set(Nfac)
        print('Nfac: {}'.format(Nfac))
        ...
        for nfac in Nfac:
            ...
            # Plot
            METRICS_DIR = '{}/metrics/Nfac{}/'.format(results_dir, nfac)
            ...
            plt.savefig('{}/lpml.pdf'.format(METRICS_DIR), bbox_inches='tight', markersize=ms)
            plt.close()
