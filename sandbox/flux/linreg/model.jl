...
function loglike(y::Vector{S}, x::Vector{S}, b0::T, b1::T, sig::T) where {T, S}
...
function elbo(y::Vector{T}, x::Vector{T}, vp) where {T}
  b0 = rsample(vp.b0)
  b1 = rsample(vp.b1)
  log_sig = rsample(vp.log_sig)
  sig = exp(log_sig)

  return loglike(y, x, b0, b1, sig) + log_p(b0, b1, log_sig) - log_q(b0, b1, log_sig, vp)
end
